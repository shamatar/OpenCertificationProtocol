...
let sha3512 = require('js-sha3').sha3_512;
let sha3384 = require('js-sha3').sha3_384;
let sha3256 = require('js-sha3').sha3_256;
let sha3224 = require('js-sha3').sha3_224;
let crypto = require('crypto');
let ethUtil = require('ethereumjs-util');

const PaddableMerkleTree = function(treeOptions) {
  // in case 'new' was omitted
  if (!(this instanceof PaddableMerkleTree)) {
    return new PaddableMerkleTree(treeOptions);
  }
  ...
  let hashFunction = function(value) {
    switch (hashType) {
      case 'SHA3-224':
  ...
  this.makeTree = function(doubleHash) {
    tree.isReady = false;
    let leafCount = tree.leaves.length;
  ...
  this.makePaddedTree = function(paddingBuffer) {
    tree.isReady = false;
    let leafCount = tree.leaves.length;
    if (leafCount > 0) { // skip this whole process if there are no leaves added to the tree
      tree.levels = [];
      tree.levels.unshift(tree.leaves);
      while (tree.levels[0].length > 1) {
        tree.levels.unshift(_calculatePaddedNextLevel(paddingBuffer));
      }
    }
    tree.isReady = true;
  };
  ...
  this.getProof = function(index, asBinary) {
    ...
    let currentRowIndex = tree.levels.length - 1;
    ...
    for (let x = currentRowIndex; x > 0; x--) {
      let currentLevelNodeCount = tree.levels[x].length;
      ...
      if (asBinary) {
        proof.push(new Buffer.from(isRightNode ? [0x00] : [0x01]));
        proof.push(tree.levels[x][siblingIndex]);
      } else {
        let sibling = {};
        let siblingPosition = isRightNode ? 'left' : 'right';
        let siblingValue = tree.levels[x][siblingIndex].toString('hex');
  ...
  this.validateBinaryProof = function(proof, targetHash, merkleRoot) {
    ...
    let proofHash = targetHash;
    let elements = proof.length / 33;
    let zero = Buffer.alloc(1);
    let one = Buffer.from('01', 'hex');
    for (let x = 0; x < elements; x++) {
      let subProof = proof.slice(x*33, (x+1)*33);
      let leftOrRight = subProof.slice(0, 1);
      let element = subProof.slice(1, 33);
  ...
  function _calculatePaddedNextLevel(paddingBuffer) {
    let nodes = [];
    let topLevel = tree.levels[0];
    let topLevelCount = topLevel.length;
...
module.exports = PaddableMerkleTree;
